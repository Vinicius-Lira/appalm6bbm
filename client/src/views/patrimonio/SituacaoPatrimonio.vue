    ...
    methods: {
                ...
                this.axios.get(process.env.VUE_APP_URL_API + '/permissao/' + localStorage.getItem("usuarioAppB4")).then(response => {
                    if(response.data) {
                        if(response.data.cadastrosApagar) {
                            this.axios.delete(process.env.VUE_APP_URL_API + '/situacaoPatrimonio/' + item.id + "/delete").then(response => {
                                if(response.data){
                                    this.snackbar = true;
                                    this.color = 'success';
        ...
        save () {
            if (this.editedIndex > -1) {
                this.axios.put(process.env.VUE_APP_URL_API + '/situacaoPatrimonio', this.editedItem).then(response => {
                    if(response.data){
                ...
                if(this.validaCampos()){
                    this.axios.post(process.env.VUE_APP_URL_API + '/situacaoPatrimonio', this.editedItem).then(response => {
                        if(response.data.id){
